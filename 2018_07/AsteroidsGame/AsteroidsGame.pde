...
boolean runBest = true; //true if replaying the best ever game
boolean humanPlaying = false; //true if the user is playing
void setup() {//on startup
  size(1200, 675);

  humanPlayer = new Player();
  pop = new Population(500);// create new population of size 200
  frameRate(speed);
...
void draw() {
  ...
  for(int i = 0;i < 1;i++){
    ...
    if (runBest&&pop.gen>0) {// if replaying the best ever game
...
void showScore() {
  if (humanPlaying) {
    ...
    if (runBest&&pop.gen>0) {
      ...
      textAlign(LEFT);
      int count = 0;
      for (int i = 0; i< pop.players.length; i++) {
        if (!pop.players[i].dead) {
          count++;
        }
      }
      text("Count: " + count, 80, 60);
      text("Gen: " + pop.gen, width-200, 60);
    }
}